...
5. aof/rdb 持久化 性能/延迟的权衡
   ...
   2. AOF+fsync+everysec 每秒执行一次，推荐(也可能会造成阻塞的问题,[当执行bgrewrite对aof进程归并优化时，redis出现被阻塞的问题](/database/redis-持久化策略#1))
...
fdatasync: 刷新内核文件缓冲区的文件到磁盘上，相比fsync要[高效](/devops/sync-fsync-fdatasync#性能)
...
可以使用[strace](/devops/strace)查看进程对fdatasync/write的调用
```
