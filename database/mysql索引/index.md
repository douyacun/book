---
Title: mysql索引
LegacyId: 8d780ba492044002a70a069ee1cff724
Slug: mysql索引
Category: database
Summary: 'innodb是索引存储表，表数据按主键顺序存放 [toc] 聚簇索引、辅助索引、联合索引 ## 聚簇索引 聚集索引按照每张表的主键构造一棵B+树，叶子节点存放的是整张表的行记录数据，叶子节点被称为数据页。
  每张表只能按照一颗B+树排序，每张'
SeoTitle: mysql索引
SeoDescription: 'innodb是索引存储表，表数据按主键顺序存放 [toc] 聚簇索引、辅助索引、联合索引 ## 聚簇索引 聚集索引按照每张表的主键构造一棵B+树，叶子节点存放的是整张表的行记录数据，叶子节点被称为数据页。
  每张表只能按照一颗B+树排序，每张'
