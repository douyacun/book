...
3. 前端只展示普通按钮时，很难判断颜色、倾斜角、字号这些关键信息。
...
最终，明显像水印的候选会默认勾选，不确定候选也会返回，但不会默认删除。
...
1. 候选展示尽量保留颜色、倾斜角和字号，一眼看出这是不是页面里的那层水印。
2. 展示态不完全照搬原始低透明度，而是做可读性增强，避免文字太淡看不清。
3. 选中的文字增加删除线，明确知道哪些字会被清理。
4. 增加全选、取消全选、拖拽框选和双击同类切换，减少“一字一字点”的成本。
5. 增加处理结果截图和成功/失败反馈，能快速确认效果。
